...
const postAuditLog = async (data) => {
  try {
    const resp = await axios({
      method: "post",
      url: `${config.baseUrl}/rt/auditlogs`,
      data,
    });
...
client.on(Events.GuildAuditLogEntryCreate, async (auditLog) => {
  ...
  const isValidOperation =
    currentChange.key === ADD || currentChange.key === REMOVE;
...
async function startBot() {
  ...
  while (retries < MAX_RETRIES) {
    try {
      if (!config.botToken) {
        throw new Error("Bot token is missing in the configuration");
      }
      await client.login(config.botToken);
      log.message("[LOG] Bot successfully logged in");

      // Start the server after successful login
      const port = config.port || 8142;
      app.listen(port, () => {
        log.message(`[LOG] Server is running on port ${port}`);
      });
      ...
      retries++;
      log.error(
        `[ERROR] Failed to log in (Attempt ${retries}/${MAX_RETRIES}):`,
        error.message
      );
      if (retries < MAX_RETRIES) {
        log.message(`[LOG] Retrying in ${RETRY_DELAY / 1000} seconds...`);
        await new Promise((resolve) => setTimeout(resolve, RETRY_DELAY));
      } else {
        log.error("[ERROR] Max retries reached. Unable to start the bot.");
